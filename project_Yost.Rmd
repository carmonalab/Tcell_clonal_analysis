...
clonotypeBias(yost.large, cloneCall = "cdr3s_pat", split.by = "patient",
              group.by = "functional.cluster", min.expand = 5) +
  scale_color_manual(values = ref.cd8@misc$atlas.palette)
...
```{r fig.width=5, fig.height=3}
biased.clones <- head(most.biased$ncells, 6)
names(biased.clones) <-  head(most.biased$Clone, 6)

plots <- list()
for (i in 1:length(biased.clones)) {
  
  ctype <- names(biased.clones)[i]
  
  cells.in.clone <- which(yost.merged[["cdr3s_pat"]]==ctype)
  plots[[i]] <- plot.projection(ref.cd8, yost.merged[,cells.in.clone],
                                pointsize = 0.3, linesize = 0.3) + 
    ggtitle(sprintf("Clone %i - size %i", i, biased.clones[i])) + NoLegend()
}
