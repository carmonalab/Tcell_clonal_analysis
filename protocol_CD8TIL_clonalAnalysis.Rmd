...
a <- plot.projection(ref.cd8, query = yost.projected[[which.patient]],
                     pointsize = 0.3, linesize = 0.3) + NoLegend() +
...
yost.merged$is.cycling <- ifelse((yost.merged$cycling.score.G1_S > 0.1 |
                                 yost.merged$cycling.score.G2_M > 0.1),
                                 yes = "Proliferating",
                                 no = "Resting")

clonotypes <- table(yost.merged$cdr3s_pat)
expanded <- names(clonotypes)[clonotypes>=2]

prolif.freq <- sapply(expanded, function(x) {
  sub <- subset(yost.merged[[]], subset=cdr3s_pat == x)
...
for (ctype in names(frequency.proliferating)) {
  ...
  plots[[ctype]] <- plot.projection(ref.cd8, yost.merged[,cells.in.clone],
                                pointsize = 2, linesize = 0.3) + 
